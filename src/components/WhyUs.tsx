  ...
  const data = [
    {
      ...
      image: (
        <svg
          className="colored w-24 h-24 text-gray-800 group-hover:text-white dark:text-white"
          aria-hidden="true"
    ...
    },
    {
      ...
      image: (
        <svg
          className="w-24 h-24 text-gray-800 group-hover:text-white dark:text-white"
          aria-hidden="true"
          xmlns="http://www.w3.org/2000/svg"
          fill="none"
          viewBox="0 0 24 24"
  ...
  return (
    <div className="min-h-screen justify-center items-center h-full w-full flex flex-col">
      ...
      <div className="w-full py-14 sm:px-0 gap-8 sm:gap-8 flex flex-wrap justify-center">
        {data.map((item, index) => (
          <WhyUsCard key={index} text={item.text} ImageSvg={item.image} />
        ))}
      </div>
    </div>
  );
...
function WhyUsCard({ text, ImageSvg }: { text: string; ImageSvg: any }) {
  return (
    <div
      className="group min-h-[280px] max-h-[280px] max-w-[280px] min-w-[280px] sm:min-h-[320px] sm:max-h-[320px] sm:max-w-[320px] sm:min-w-[320px]
