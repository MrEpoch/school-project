...
<div className="min-h-screen dark:bg-darkmode-500 py-16">
	<div className="p-4 mx-auto max-w-screen-xl h-full w-full flex flex-col gap-[2rem]">
      <Image src={sponsorship.image_url} width={1000} height={750} alt="" className="w-full" />
        ...
        <p className="sm:text-lg text-sm">{sponsorship.expires_at.toDateString()}</p>
			</div>
			<div className="flex gap-2 justify-between">
		...
		<div className="w-full">
			...
			<p className="mt-4 text-lg text-gray-900 dark:text-white">{sponsorship.description}</p>
		</div>
		<span className="long-line bg-gradient-to-r from-lime-400 to-green-400 h-1 w-full" />
    <div className="w-full flex gap-6 justify-end">
      <Link className="transition bg-black hover:bg-blue-700 text-white font-bold flex justify-center items-center p-4 rounded" href={`/auth/sponsorship/update/${sponsorship.id}`}>
