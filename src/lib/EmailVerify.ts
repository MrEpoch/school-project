...
export async function verifyEmailCode(user: User, code: string): Promise<boolean> {
  try {
    ...
    if (!isWithinExpirationDate(email_verify_code.expires_at) || email_verify_code.email !== user.email) {
      return false
    } 
